...
// The brand logo identifies the engine; the right-hand dot carries the verdict.
export function Engines({ engines }: { engines: EngineReadiness[] }) {
  return (
    <div className="border-t border-line px-4 py-3">
      <div className="eyebrow mb-2.5">Per-engine readiness</div>
      <ul className="flex flex-col gap-2.5">
        {engines.map((e) => (
          <li key={e.id} className="flex items-start gap-2.5">
            <EngineLogo id={e.id} size={16} />
            ...
            <span
              className={`mt-1 h-2 w-2 shrink-0 rounded-full ${DOT[engineTone(e.status)]}`}
              aria-hidden
            />
          </li>
        ))}
